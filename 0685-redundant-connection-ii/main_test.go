...
func Test_findRedundantDirectedConnection(t *testing.T) {
	...
	tests := []struct {
		...
		{args: args{edges: [][]int{{1, 2}, {2, 3}, {3, 4}, {4, 1}, {1, 5}}}, want: []int{4, 1}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := findRedundantDirectedConnection1(tt.args.edges); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("findRedundantDirectedConnection1() = %v, want %v", got, tt.want)
			}
		})
	}
}
